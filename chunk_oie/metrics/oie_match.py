...
class OIE_Matcher(Metric):
    ...
    def __call__(self, tokens: list = None, prediction: list = None, predicate_id: list = None,
    ...
    def get_metric(self, reset: bool = False):
        if reset:
            ret = defaultdict(lambda: {})
            sents = []
            for tokens, predictions, predicate_id, confidences in zip(self._all_tokens,  self._all_predictions,
                                                                      self._all_predicate_id, self._all_confidences):
                extractions = [(x, y) for x, y in zip(predictions, confidences)]
                ...
                if predicate_id:
                    ...
                    try:
                        predicate_id = predictions.index("V")
                        pred_word = tokens[predicate_id]
                        ret[cur_sent][pred_word] = (((predicate_id, pred_word), extractions),)
                    except:
                        continue
            ...
            final_F1, my_auc, best_F1, auc = b.compare(predicted=predicted.oie, matchingFunc=Matcher.syntacticHeadMatch,
                                                    output_fn=output_pr_file)

            self._epoch_num += 1
            self.reset()
            return {'final_f1': final_F1, 'final_auc': my_auc, 'best_f1': best_F1, 'auc': auc}

        else:
            return {'final_f1': 0.0, 'final_auc': 0.0, 'best_f1': 0.0, 'auc': 0.0}
...
class Extraction:
    ...
    def __str__(self):
        ...
        """
        return '\t'.join(map(str, [' '.join(self.sent), self.prob, self.pred,
